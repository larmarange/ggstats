...
By default, totals are added on each side of the plot. In case of an uneven number of answer levels, the central level is not taken into account for computing totals. With `totals_include_center = TRUE`, hal of the proportion of the central level will be added on each side.
...
Or, you could use `exclude_fill_values` to not display specfic values, but still counting them in the denominator for computing proportions.
