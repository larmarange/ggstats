...
Expand scale limits to make them symmetric around zero. Can be passed as
argument to parameter `limits` of continuous scales from packages
`{ggplot2}` or `{scales}`. Can be also used to obtain an enclosing
...
- x:
  ...
  a vector of numeric values, possibly a range, from which to compute
  enclosing range
